...
const METHOD = [
  {
    ...
    body:
      "Tuition is the easy part. We cover materials, transport and a modest stipend, paired with monthly mentoring — so a girl from a first-generation household doesn’t drop out the first time a fee notice arrives.",
  },
  {
    n: "02",
    icon: Stethoscope,
  ...
  },
  {
    n: "03",
    icon: Coins,
    title: "Place the worker back inside her own community.",
    body:
      "Trained women are recruited, certified and placed as paid community health workers with health systems or NGO partners. The community gains capacity; she earns a livelihood; both compound.",
  },
] as const;
  ...
  return (
    <>
      <PageHero
        ...
        lead="Most ‘education programmes’ end the day the certificate is handed over. We design ours to end the day she draws her first salary — inside the community she grew up in."
      >
          ...
          <div className="grid lg:grid-cols-2 gap-5 lg:gap-6">
            {PROGRAMMES.map((p, i) => (
              <FadeUp key={p.href} delay={140 + i * 90}>
                <Link
                  to={p.href}
                  className="group block h-full overflow-hidden rounded-2xl bg-snow border border-hairline shadow-[0_1px_2px_rgba(28,0,96,0.04),0_8px_24px_-16px_rgba(28,0,96,0.12)] transition-[border-color,box-shadow,transform] duration-300 ease-out hover:-translate-y-1 hover:border-ink/40 hover:shadow-[0_4px_8px_rgba(28,0,96,0.06),0_22px_44px_-22px_rgba(28,0,96,0.32)] active:scale-[0.995]"
                >
